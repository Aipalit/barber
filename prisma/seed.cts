...
async function main() {
  ...
  const creativeNames = ["Barbearia Vintage", "Corte & Estilo", "Barba & Navalha", "The Dapper Den", "Cabelo & Cia.", "Machado & Tesoura", "Barbearia Elegance", "Aparência Impecável", "Estilo Urbano", "Estilo Clássico"];
  const addresses = ["Rua da Barbearia, 123", "Avenida dos Cortes, 456", "Praça da Barba, 789", "Travessa da Navalha, 101", "Alameda dos Estilos, 202", "Estrada do Machado, 303", "Avenida Elegante, 404", "Praça da Aparência, 505", "Rua Urbana, 606", "Avenida Clássica, 707"];
  const services = [
  ...
  // 2. Limpando o banco de dados para o script ser re-executável
  await prisma.barbershopService.deleteMany();
  await prisma.barbershop.deleteMany();

  // 3. Criando as barbearias e seus serviços em uma única transação por barbearia
  for (let i = 0; i < 10; i++) {
    await prisma.barbershop.create({
      data: {
        ...
        // 4.Criando os serviços junto com a barbearia
        services: {
          createMany: {
